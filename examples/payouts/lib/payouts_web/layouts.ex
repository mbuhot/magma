defmodule PayoutsWeb.Layouts do
  @moduledoc """
  ...
  * { box-sizing: border-box; }
  ...
  def root(assigns) do
    ...
    <html lang="en">
      <head>
        ...
        <.live_title default="payouts console">{assigns[:page_title]}</.live_title>
  ...
  def app(assigns) do
    ...
    <header>
      <b>payouts</b>
      <span>a magma console — every row below is a checkpoint in Postgres</span>
      <span style="margin-left:auto"><.link navigate={~p"/"}>all transfers</.link></span>
    </header>
  ...
  @doc "Cents as an amount, with the sign kept."
  @spec money(integer()) :: String.t()
  def money(cents) do
    sign = if cents < 0, do: "-", else: ""
    cents = abs(cents)

    "#{sign}#{div(cents, 100)}.#{cents |> rem(100) |> to_string() |> String.pad_leading(2, "0")}"
  end
end
